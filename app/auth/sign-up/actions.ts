...
export async function signup(formData: FormData) {
  ...
  const customDesignation = (formData.get('customDesignation') as string | null)?.trim() ?? ''
  ...
  if (!firstName || !lastName || !designation || !email || !password) {
    console.error('Missing required fields:', { firstName: !!firstName, lastName: !!lastName, designation: !!designation, email: !!email, password: !!password })
    redirect('/error')
  }
  ...
  try {
    ...
    const canonicalDesignation = isOthers ? (customDesignation || 'Others') : designation
    ...
    const profileRow = {
      ...
      last_name: lastName,
      email, // convenience copy (canonical is in auth.users)
      designation: canonicalDesignation || null,
      is_director: isDirector,
      is_branch_manager: isBranchManager,
      is_others: isOthers,
      // department: null for director, otherwise use provided value or null
